...
public class mainWindow extends JComponent implements KeyListener, ActionListener, Observer
{
	...
	//消除行时的动画	
	private void drawRowAnimation(Graphics g, int col) throws IOException
	{
		try
		{
			FileInputStream fileau=new  FileInputStream("res/clear.wav");
	...
	private void drawScoring(Graphics g)
	{
		...
		//g.drawString("对手分数: " + gameInformation.getScore()+100, Drawer.next_offset_x, 340);
	}
	...
	private void recordScore()
	{
		...
		if(inputResult != null)
		{
			...
			if(Keeper.addScore(gameInformation))
			{
				...
				try {
					fileau = new  FileInputStream("res/high-score.wav");
					AudioStream as = new AudioStream(fileau);
					AudioPlayer.player.start(as);
				}
				catch (IOException e) {
